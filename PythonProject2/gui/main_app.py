...
# Importy absolutne z katalogu głównego projektu
...
class HangmanApp(tk.Tk):
    ...
    def start_single_player_game(self):
        word, category = crud.get_random_word()
        if not word: messagebox.showerror("Błąd", "Brak haseł w bazie danych!"); return
        game_instance = SinglePlayer(word, self.current_user.id);
        game_frame = self.frames[GameFrame];
        game_frame.setup_game(game_instance, word, category, 'single');
    ...
    def start_two_player_game(self):
        ...
        game_frame = self.frames[GameFrame];
        game_frame.setup_game(game_instance, word, category, 'two', self.current_user.username,
                              self.player2_user.username);
        self.show_frame(GameFrame)

    def logout(self):
        self.current_user = None; self.player2_user = None; self.show_frame(LoginFrame)
...
class NotebookFrame(tk.Frame):
    ...
    def draw_notebook_background(self, event=None):
        ...
        for i in range(0, height, 25): self.canvas.create_line(0, i, width, i, fill="#a0c0ff", tags="background")
...
class LoginFrame(NotebookFrame):
    ...
    def login(self):
        ...
        if user:
            self.controller.current_user = user; self.controller.show_frame(MainMenuFrame); self.password_entry.delete(
                0, 'end')
        else:
            messagebox.showerror("Błąd", "Nieprawidłowa nazwa użytkownika lub hasło.")

    def register(self):
        username = self.username_entry.get(); password = self.password_entry.get(); success, message = auth.register_user(
            username, password); messagebox.showinfo("Rejestracja", message)
...
class Player2LoginFrame(NotebookFrame):
    ...
    def login_player2(self):
        username = self.username_entry.get();
        password = self.password_entry.get()
        if username == self.controller.current_user.username: messagebox.showerror("Błąd",
                                                                                   "Gracz 2 nie może być tą samą osobą co Gracz 1."); return
        user = auth.login_user(username, password)
        if user:
            self.controller.player2_user = user; self.password_entry.delete(0,
                                                                            'end'); self.controller.start_two_player_game()
        else:
            messagebox.showerror("Błąd", "Nieprawidłowa nazwa użytkownika lub hasło.")
...
class MainMenuFrame(NotebookFrame):
    ...
    def on_show(self):
        if self.controller.current_user: self.welcome_label.config(
            text=f"Witaj, {self.controller.current_user.username}!")
...
class GameFrame(NotebookFrame):
    ...
    def setup_game(self, game_instance, word, category, mode, p1_name=None, p2_name=None):
        ...
        self.word_label = tk.Label(self.canvas, text=self.game.game.get_display_word(), font=(FONT_FAMILY, 30),
                                   bg=NOTEBOOK_BG)
        self.word_label.place(relx=0.6, rely=0.2, anchor="center")
        tk.Label(self.canvas, text=f"Kategoria: {category}", font=(FONT_FAMILY, 14), bg=NOTEBOOK_BG).place(relx=0.6,
                                                                                                           rely=0.3,
                                                                                                           anchor="center")
        self.lives_label = tk.Label(self.canvas, font=(FONT_FAMILY, 14), bg=NOTEBOOK_BG, justify="center")
        self.lives_label.place(relx=0.6, rely=0.4, anchor="center")
    ...
    def handle_guess(self, letter):
        ...
        if result == "incorrect":
            # ZMIANA: Przekazujemy liczbę utraconych żyć, co jest bardziej niezawodne
            lives_lost = self.game.game.max_lives - self.game.lives if self.mode == 'single' else (
                                                                                                              self.game.game.max_lives // 2 * 2) - (
                                                                                                              self.game.players[
                                                                                                                  1][
                                                                                                                  'lives'] +
                                                                                                              self.game.players[
...
class StatsFrame(NotebookFrame):
    ...
    def on_show(self):
        ...
        for i, user in enumerate(users, 1): self.tree.insert("", "end", values=(
        i, user.username, user.games_played, user.games_won))